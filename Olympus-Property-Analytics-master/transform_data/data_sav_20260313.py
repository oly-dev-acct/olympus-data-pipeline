...
class IncomeStatementDataNew(Data):
    def structurize_data(self):
        raw_rows = self.read_csv_file()
        records: list[dict] = []
        ...
        while i < len(raw_rows):
            cells = raw_rows[i].split(",")
 
            # Property header 
            if cells[0].strip().startswith("Property:"):
                property_name = cells[0].strip().removeprefix("Property:").strip()
                if i + 1 < len(raw_rows):
                    next_cells = raw_rows[i + 1].split(",")
                    ao_raw = next_cells[0].strip().removeprefix("As Of:").strip()
                    snapshot_date = self.parse_date(ao_raw) or ao_raw
                i += 1          # skip "As Of:" line
                month_columns = []
                col_indices   = []
                i += 1
            ...
            # Month Ending date row 
            date_hits = [
                (idx, self.parse_date(c))
                for idx, c in enumerate(cells)
                if self.parse_date(c)
            ]
            if date_hits and property_name:
                month_columns = [d for _, d in date_hits]
                col_indices   = [idx for idx, _ in date_hits]
                i += 1
                continue
 
            # Skip noise rows
            first = cells[0].strip()
            if first in ("", "Actual", "Budget", "Summary", "12 Month", "Month To Date",
                         "Month Ending", "All Locations"):
                i += 1
                continue
 
            # Data rows 
            if month_columns and property_name:
                category = first
                if not category:
                    i += 1
                    continue
                has_value = any(
                    self.to_number(cells[ci]) is not None
                ...
                if not has_value:
                    i += 1
                    continue
                for ci, month_end in zip(col_indices, month_columns):
                    if ci >= len(cells):
                        continue
                    records.append({
                        "propertyname":   property_name,
                        "snapshotdate":   snapshot_date,
                        "enddateofmonth": month_end,
                        "category":       category,
                        "value":          self.to_number(cells[ci]),
        ...
        self.df = pd.DataFrame(
            records,
            columns=["propertyname", "snapshotdate", "enddateofmonth", "category", "value", "insert_date"]
        )
        logging.info(
            f"Income File data has been scrubbed successfully — "
            f"{len(self.df):,} rows, {self.df['propertyname'].nunique()} properties"
        )
